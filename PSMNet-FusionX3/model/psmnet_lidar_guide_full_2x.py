...
class PSMNetLiDARGuideFull2x(LightningModule):
    def __init__(self, maxdisparity=192, num_F=32, channel=3, norm_mode='categorical'):
        super(PSMNetLiDARGuideFull2x, self).__init__()
        ...
        self.count_levels = 1
        self.layer2d = feature_extraction2x(channel + 1)
    ...
    def forward(self, inputs, mode='train'):
        ...
        #print('size: ')
        #print(hints.size())
        hints = nn.functional.upsample(hints, [int(hints.size()[2]/SUBSAMPLE), int(hints.size()[3]/SUBSAMPLE)], mode='nearest')
        #validhints = validhints.unsqueeze(1)
        validhints = nn.functional.upsample(validhints, [int(validhints.size()[2]/SUBSAMPLE), int(validhints.size()[3]/SUBSAMPLE)], mode='nearest')
        ...
        hints = hints.unsqueeze(2).expand(-1, features, num_disp, -1, -1)
        validhints = validhints.unsqueeze(2).expand(-1, features, num_disp, -1, -1)
        ...
        if not self.training :
            del hints,validhints, disparities
