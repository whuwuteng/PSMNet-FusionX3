...
from dataset.dataset_toulouseEx_lightning_weight import DatasetToulouseWeight_lightning
from model.psmnet_lidar_weight_lightning import PSMNetLiDARWeight1x_lightning
...
def main():
    ...
    model = PSMNetLiDARWeight1x_lightning(args.maxdisp, norm_mode = norm_mode)

    train_output_size = (256, 512)
    val_output_size =  (256, 512) # NOTE: set to (256, 1216) if there is enough gpu memory

    data = DatasetToulouseWeight_lightning(args.train_datapath, train_output_size, args.val_datapath, val_output_size, args.guideL, args.guideR, normalize, train_batch_size = 3, val_batch_size = 1)

    checkpoint_callback = ModelCheckpoint(dirpath=args.savemodel, save_top_k = -1, every_n_epochs = 5, filename='{epoch:03d}')

    trainer = Trainer(devices=4, accelerator="gpu", strategy="ddp", max_epochs=800, resume_from_checkpoint=args.loadmodel,callbacks=[checkpoint_callback], enable_progress_bar=False,logger=False)
