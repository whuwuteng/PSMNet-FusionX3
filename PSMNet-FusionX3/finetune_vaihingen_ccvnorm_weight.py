...
train_dataset = DatasetToulouseWeight(args.train_datapath, train_output_size, 'train', args.guideL, args.guideR, normalize)
val_dataset = DatasetToulouseWeight(args.val_datapath, val_output_size, 'val', args.guideL, args.guideR, normalize)

# batchsize is too small
# now batchsize doesnot deteriorate the perfromance  
...
def train(inputs, disp_true):
    ...
    loss1 = F.smooth_l1_loss(output1[mask], disp_true[mask], reduction='none')
    loss1 = torch.mean(loss1 * left_weight[mask])
    loss2 = F.smooth_l1_loss(output2[mask], disp_true[mask], reduction='none')
    loss2 = torch.mean(loss2 * left_weight[mask])
    loss3 = F.smooth_l1_loss(output3[mask], disp_true[mask], reduction='none')
...
def main():
    ...
    for epoch in range(args.epoch_start, args.epochs+1):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            inputs['left_cmp'] = data['left_cmp']
            inputs['right_cmp'] = data['right_cmp']
            target = data['left_d']
