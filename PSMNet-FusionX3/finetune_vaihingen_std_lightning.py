...
def main():
    ...
    data = KITTILoader_std_lightning.myImageFloderStd_lightning(all_left_img, all_right_img, all_left_disp, test_left_img, test_right_img, test_left_disp, normalize, train_batch_size = 8, val_batch_size = 1)

    # not used for all
    checkpoint_callback = ModelCheckpoint(dirpath=args.savemodel,  save_top_k = -1, every_n_epochs = 5, filename='{epoch:03d}')

    trainer = Trainer(devices=2, accelerator="gpu", strategy="ddp", resume_from_checkpoint=args.loadmodel, callbacks=[checkpoint_callback], enable_progress_bar=False,logger=False)
