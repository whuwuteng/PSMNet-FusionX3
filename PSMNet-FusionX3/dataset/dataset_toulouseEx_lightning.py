...
class DatasetToulouse_lightning(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.trainset = DatasetToulouse(self.datalist_train, self.train_output_size, 'train', self.guideL_folder, self.guideR_folder, normalize=self.normalize)
            self.valset = DatasetToulouse(self.datalist_val, self.val_output_size, 'val', self.guideL_folder, self.guideR_folder, normalize=self.normalize)
        else:
