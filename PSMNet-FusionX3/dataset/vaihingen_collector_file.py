...
# inorder to load the base height information
def load_vaihingen_dataEx(folderlist):
    """load current file from the list"""
    file_path, filename = os.path.split(folderlist)
...
def datacollectorallEx(train_filelist, val_filelist):

    left_train, right_train, disp_train_L, bh_ratio_train = load_vaihingen_dataEx(train_filelist)
    left_val, right_val, disp_val_L, bh_ratio_val = load_vaihingen_dataEx(val_filelist)

    return left_train, right_train, disp_train_L, bh_ratio_train, left_val, right_val, disp_val_L, bh_ratio_val