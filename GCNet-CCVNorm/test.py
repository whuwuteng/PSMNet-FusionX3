...
def main():
    ...
    if args.dataset == 'kitti2017':
        dataset = DatasetKITTI2017(args.rgb_dir, args.depth_dir, 'val', #'my_test',
    ...
    with torch.no_grad():
        for it, data in enumerate(pbar):
            ...
            if cfg.to_disparity:
                ...
                if args.save :
                    #index2_dir = inputs['left_rgb'].rfind('/')
                    pred_disp_img = (torch.squeeze(pred_disp)).data.cpu().detach().numpy()
                    pred_disp_img = (pred_disp_img * 256).astype('uint16')
                    #print(pred_disp_img)
                    filename = cfg.workspace + "/%06d.png"%it #inputs['left_rgb'][index2_dir: len(inputs['left_rgb'])]
                    #print(filename)
                    imageio.imwrite(filename, pred_disp_img)
            else:
                raise NotImplementedError
