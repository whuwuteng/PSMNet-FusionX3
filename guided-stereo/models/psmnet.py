...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right, hints, validhints, k, c):
        ...
        if self.useguide:
            ...
            hints = F.upsample(hints, [int(hints.size()[2]/SUBSAMPLE), int(hints.size()[3]/SUBSAMPLE)], mode='nearest').squeeze(1)
            validhints = validhints.unsqueeze(1)
            validhints = F.upsample(validhints, [int(validhints.size()[2]/SUBSAMPLE), int(validhints.size()[3]/SUBSAMPLE)], mode='nearest').squeeze(1)
